...
import {
  ...
  fetchAdminUnreadNotificationCount,
...
import { requestNotificationPermissionAndRegister } from "@/components/notifications/FcmProvider";
...
export function AdminNotificationBellDropdown() {
  ...
  const refreshCount = useCallback(async () => {
    try {
      setCount(await fetchAdminUnreadNotificationCount());
    } catch {
      setCount(0);
    }
  }, []);
  ...
  const loadRecent = useCallback(async () => {
    setLoading(true);
    try {
      const result = await fetchAdminNotifications({ limit: 8 });
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {count > 0 ? (
          <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-indigo-400 px-1 text-[10px] font-bold text-indigo-950">
            {badgeLabel}
          </span>
        ) : null}
      </button>
        ...
        <div className="absolute right-0 top-full z-50 mt-2 w-[min(100vw-2rem,22rem)] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                ...
                {items.map((item) => {
                  ...
                  return (
                    <li key={item.id} className="border-b border-slate-50 last:border-0">
                      <Link
                        href={href}
                        className={`block px-4 py-3 hover:bg-indigo-50/50 ${unread ? "bg-indigo-50/30" : ""}`}
                        onClick={() => {
                          setOpen(false);
                          if (unread) {
          ...
          <div className="border-t border-slate-100 px-4 py-2">
            <Link
              ...
              className="block text-center text-xs font-semibold text-indigo-700 hover:underline"
